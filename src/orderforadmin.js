...
//   console.log(action);
  $.ajax({
      method: "GET",
      url: "controlr.php",
      data: { id: pid, action: "changestatus"},
      //  dataType: "JSON"
  }).done(function (data) {
...
//   let cat = $("#option").val();
  ...
  displayAllOrders(gData, currpage);
});

function displayAllOrders(data, pageno) {
  ...
  for (let i = ps; i <pe; i++) {
    ...
    str +=
      ...
      "</td>\
        <td>" +
      "<a href=" +
      " # class='btn btn-success deletebtn' data-action=true  data-pid=" +
      obj.order_id +
      ">aprove</a></td>\
        </tr>";
  }
